...
select
  ...
  coalesce(cast(cc.amount as numeric), cast(thb.JPY as numeric)) as amount, -- we want the cc amount here
  ...
  coalesce(thb.payment_method, cc.payment_method) as payment_method,
  coalesce(thb.payment_method, cc.payment_method_complete) as payment_method_complete,
  coalesce(thb.payee, cc.payee) as payee,
  coalesce(thb.payee, cc.payee_standardized) as payee_standardized,
  coalesce(thb.item, cc.item) as item, 
  coalesce(thb.category, cc.category) as category,
  coalesce(thb.category, cc.category_standardized) as category_standardized, 
  coalesce(thb.category, cc.category_complete) as category_complete, 
  coalesce(thb.tags, cc.tags) as tags,
  coalesce(thb.tags, cc.tags_complete) as tags_complete, 
  ...
  coalesce(thb.social, cc.social_complete) as social_complete, 
  coalesce(thb.store_type, cc.store_type) as store_type, 
  coalesce(thb.store_type, cc.store_type_standardized) as store_type_standardized, 
  coalesce(thb.store_type, cc.store_type_complete) as store_type_complete, 
  coalesce(thb.purchase_channel, cc.purchase_channel) as purchase_channel,
  coalesce(thb.purchase_channel, cc.purchase_channel_complete) as purchase_channel_complete, 
  coalesce(thb.essentiality, cc.essentiality) as essentiality,
  coalesce(thb.essentiality, cc.essentiality_complete) as essentiality_complete,
  coalesce(thb.recurrence_type, cc.recurrence_type) as recurrence_type,
  coalesce(thb.recurrence_type, cc.recurrence_type_complete) as recurrence_type_complete, 
  coalesce(thb.value_rating, cc.value_rating) as value_rating,
  coalesce(thb.value_rating, cc.value_rating_complete) as value_rating_complete, 
